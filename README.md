...
## Interesting Ncode discoveries

This doesn't really have anything to do with this repository; it's just some
things I discovered about Ncode while using it that you may find interesting.

Ncode's documentation is very strict about being careful about scaling and
size, but the actual device is extremely resilient to incorrect printing. I
accidentally applied A4 Ncode to a letter-sized sheet (stretched and
distorted!), and it worked just fine, with the writing stretched and distorted
as expected. Re-distorting it back resulting in perfectly usable scribbles. I
haven't gone so far as printing A5 Ncode on letter paper or anything like that,
but within some reasonable amount of distortion, it seems to work great.

Ncode's documentation is also very strict about colors. The PDFs generated by
the SDK (and this tool) reserve the black channel exclusively for the Ncode
itself, and print the actual content using only the CMY channels. I have a
laser printer, which is the recommendation, but it makes me wonder how precise
this has to be. To the human eye, of course, the K=0 text looks grey, but the
pen still seems to be able to distinguish perfectly. I wonder if the printer
*actually* prints K=0 parts of the image with only color ink, or if it
normalizes the color and prints it with black. And, I wonder if the camera is
actually distinguishing the not-technically-black ink, or it's just looking for
different black levels (so a black-and-white printout would still work if the
text was just grey instead of black).
